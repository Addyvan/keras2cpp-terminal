...
class GameMap:
    ...
    def in_arena_bounds(self, location):
        ...
        """
        top_half_check = self.in_top_half(location)
        
        bottom_half_check = self.in_bottom_half(location)

        return bottom_half_check or top_half_check
    
    def in_top_half(self, location):
        x, y = location
    ...
    def get_top_half(self):
        ...
        for x in range(0, self.ARENA_SIZE):
            for y in range(0, self.ARENA_SIZE):
                if self.in_top_half((x, y)):
                    half_map.append((x, y))
            # Return top half as list of tuples
        return half_map
